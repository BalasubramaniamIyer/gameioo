...
class Tilemap:
    ...
    def save(self, path):
        f = open(path, 'w')
        json.dump({'tilemap': self.tilemap, 'tile_size': self.tile_size, 'offgrid': self.offgrid_tiles }, f) #this will save the tilemap to a file
        f.close()
    ...
    def load(self, path):
        ...
        self.tilemap = map_data['tilemap'] #this will load the tilemap from a file
    ...
    def render(self, surf, offset = (0, 0)):
        ...
        for x in range(int(offset[0]) // self.tile_size, (int(offset[0]) + surf.get_width()) // self.tile_size + 1): #this will get the tiles that are on the screen
